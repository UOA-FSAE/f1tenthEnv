...
class Environment(Node):
    def __init__(self, step_duration: float = 0.0, boundary_size: float = 10):
        ...
        self.tf_data: TFMessage = TFMessage()
        self.reward_tf_data: TFMessage = TFMessage()
    ...
    def step(self, action: tuple[float, float]):
        ...
        while time.monotonic() < end_step_time:  # TODO: not sure if this works when step duration is 0
            self.spin_once()

            self.tf_data = self.tf_buffer.lookup_transform(target_frame='f1tenth/chassis', source_frame='f1tenth/odom',
    ...
    def get_reward(self) -> float:
        if sqrt(
            pow(self.tf_data.transforms.transform.translation.x - self.reward_tf_data.transforms.transform.translation.x
                , 2) +
            pow(self.tf_data.transforms.transform.translation.y - self.reward_tf_data.transforms.transform.translation.y
                , 2)
        ).real <= 0.2:
    ...
    def get_is_terminated(self) -> bool:
        ...
        if self.tf_data.transforms.transform.translation.x > self.boundary_size or \
                self.tf_data.transforms.transform.translation.y > self.boundary_size or \
                self.tf_data.transforms.transform.translation.x < -self.boundary_size or \
                self.tf_data.transforms.transform.translation.y < -self.boundary_size:
            return True
    ...
    def set_reward(self):
        self.reward_tf_data.transforms.transform.translation.x = random() * 20 - 10
        self.reward_tf_data.transforms.transform.translation.y = random() * 20 - 10

    # Callbacks --------------------------------------------------------------------------------------------------------
    def spin_once(self):
        # This is a bad fix due to this node being on a global executor
